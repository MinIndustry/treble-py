...
Python bindings for the Treble audio synthesis engine.
...
spec = treble_py.GraphSpec(
    ...
    note_off=0.5,
    duration=0.7,
    source=treble_py.SourceSpec(waveform="sine", attack=0.01, decay=0.1,
...
time from `available_filters()` and injected into the `treble_py` namespace. Each one
